...
export default async function HomePage() {
  let config = null;
  try {
    config = await getSiteConfig("en");
  } catch {
    // CMS not available yet, show static fallback
  }
    ...
    <div className="min-h-screen bg-[#080c14] text-white">
      ...
      <section className="pt-32 pb-20 px-6 text-center">
        <div className="max-w-4xl mx-auto">
          <div className="inline-block text-xs bg-[#c9a84c]/10 text-[#c9a84c] px-3 py-1 rounded-full mb-6">
            China Digital Advertising
          </div>
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6">
            One Connection.<br />
            <span className="text-[#c9a84c]">All China Platforms.</span>
          </h1>
          <p className="text-lg text-[#b3c0d8] max-w-2xl mx-auto mb-10">
            The unified media interface for international agencies entering China&apos;s digital advertising market.
            WeChat, Douyin, Baidu, Xiaohongshu, Bilibili — one partner, all platforms.
          </p>
          <div className="flex justify-center gap-4">
            <a href="/contact" className="bg-[#c9a84c] text-[#080c14] px-6 py-3 rounded-lg font-semibold hover:bg-[#e8c87a] transition">
              Start Your Campaign
            </a>
            <a href="/services" className="border border-[#1e2d45] px-6 py-3 rounded-lg text-[#b3c0d8] hover:border-[#c9a84c] hover:text-white transition">
              View Services
            </a>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {["WeChat", "Douyin", "Baidu", "Xiaohongshu", "Bilibili", "Bing China"].map((platform) => (
              <div key={platform} className="bg-[#111827] border border-[#1e2d45] px-6 py-3 rounded-lg text-sm text-[#b3c0d8]">
                {platform}
              </div>
            ))}
          </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-[#b3c0d8] mb-12 max-w-2xl mx-auto">
            Full-service China digital advertising — from strategy to execution, across all major platforms.
          </p>
      ...
      <section className="py-20 px-6 bg-[#111827]">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-[#b3c0d8] mb-8">
            Get a free consultation on which platforms and strategies fit your brand.
          </p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-sm text-[#b3c0d8]">
                {config?.footer_description || "The unified media interface for international agencies entering China's digital advertising market."}
              </p>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(config?.footer_platforms || []).map((p) => (
                  <a key={p.name} href={p.url} target="_blank" rel="noopener noreferrer"
                    className="text-xs bg-[#0d1220] border border-[#1e2d45] px-2 py-1 rounded hover:border-[#c9a84c] transition">
                    {p.name}
                  </a>
                ))}
              </div>
          ...
          <div className="border-t border-[#1e2d45] mt-8 pt-6 text-center text-xs text-[#8899b5]">
            &copy; {new Date().getFullYear()} Tuyue Media Gateway (TMG). All rights reserved.
          </div>
        </div>
      </footer>
    </div>
